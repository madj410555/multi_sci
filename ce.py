...
def train_epoch(model, train_loader, train_transform, criterion, optimizer, scheduler):
    ...
    for data, target, mel_data in train_loader:
        data, target, mel_data = data.to(args.device), target.to(args.device), mel_data.to(args.device)
...
def val_epoch(model, val_loader, val_transform, criterion):
    ...
    with torch.no_grad():
        ...
        for data, target, mel_data in val_loader:
            data, target, mel_data = data.to(args.device), target.to(args.device), mel_data.to(args.device)
            
            output = model(val_transform(data),mel_data)
            loss = criterion(output, target)
            epoch_loss += loss.item()
